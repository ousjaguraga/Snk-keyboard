...
class KeyboardViewController: UIInputViewController {
    ...
    func addKeys(for keys: [[String]]) {
        ...
        let rowVerticalPaddings: [CGFloat] = [12, 12, 12, 12]
        ...
        if let lastRow = previousRow {
            lastRow.bottomAnchor.constraint(lessThanOrEqualTo: keyboardView.bottomAnchor, constant: -rowVerticalPadding).isActive = true
        }
    }
    ...
    func addStandardRowKeys(_ rowKeys: [String], to rowView: UIView, keySpacing: CGFloat) {
        ...
        for key in rowKeys {
            ...
            keyButton.heightAnchor.constraint(equalToConstant: 44).isActive = true // Set fixed key height
    ...
    func addShiftRowKeys(_ rowKeys: [String], to rowView: UIView, keySpacing: CGFloat) {
        ...
        shiftKeyButton.heightAnchor.constraint(equalToConstant: 44).isActive = true // Set fixed key height
        ...
        deleteKeyButton.heightAnchor.constraint(equalToConstant: 44).isActive = true // Set fixed key height
    ...
    func createKeyButton(title: String) -> KeyButton {
        ...
        case "Space":
            button.setTitle("bera", for: .normal)
            ...
            button.accessibilityLabel = "bera"
        ...
        if !["Shift", "Delete", "⌫", "Globe", "Return", "Space", "123", "ABC"].contains(title) {
            button.enablePopup()
            // Set the keyPressAction closure
            button.keyPressAction = { [weak self, weak button] in
        ...
        // Update the target-action setup
        if title == "Globe" {
            button.addTarget(self, action: #selector(handleInputModeList(from:with:)), for: .allTouchEvents)
        } else {
            // Remove any existing targets first
            button.removeTarget(nil, action: nil, for: .allEvents)
            // Add the keyPressed action for buttons without popup
            if ["Shift", "Delete", "⌫", "Globe", "Return", "Space", "123", "ABC"].contains(title) {
    ...
    @objc func keyPressed(_ sender: UIButton) {
        ...
        // Get the key value from either title or accessibility label
        ...
        DispatchQueue.main.async {
            ...
            case "123", "ABC":
                self.handleKeyboardToggle()

            default:
                let character: String
                if self.keyboardState == .letters {
                    character = self.shiftState == .off ? keyPressed.lowercased() : keyPressed.uppercased()
                } else if self.keyboardState == .numbers || self.keyboardState == .symbols {
                    if self.shiftState != .off, let shiftedChar = self.numberShiftMappings[keyPressed] {
                ...
                // Insert the text using the document proxy
                proxy.insertText(character)

                // Update shift state if needed
                if self.shiftState == .on && self.keyboardState == .letters {
                    self.shiftState = .off
                    self.updateShiftState()
                }
            }

            // Update suggestions after text changes
        ...
        for button in keyButtons {
            if let key = button.accessibilityLabel {
                if key == "Shift" {
                    ...
                    switch shiftState {
                    case .off:
                        shiftImageName = keyboardState == .letters ? "shift" : "shift"
                    case .on:
                        shiftImageName = keyboardState == .letters ? "shift.fill" : "shift"
                    case .capsLock:
                        shiftImageName = "capslock.fill"
                    }
                    let shiftImage = UIImage(systemName: shiftImageName)

                    // set shift button title
                    let shiftTitle: String

                    // if keyboard type is letter use shift image else use a string
                    if keyboardState == .numbers || keyboardState == .symbols {
                        shiftTitle =  keyboardState == .numbers ? "#+=" : "123."
                        button.setTitle(shiftTitle, for: .normal)
                        button.setImage(nil, for: .normal) // Remove image
                    } else {
                        button.setImage(shiftImage, for: .normal)
                    ...
                    if shiftState == .on || shiftState == .capsLock {
                        button.backgroundColor = .white // Adjust if needed
                    } else {
                        button.backgroundColor = .functionKeyBackground
                    }
    ...
    @objc func handleSpacebarPan(_ gesture: UIPanGestureRecognizer) {
        ...
        case .began, .changed:
            let cursorMovement = Int(translation.x / 1) // Adjust sensitivity as needed
            if cursorMovement != 0 {
