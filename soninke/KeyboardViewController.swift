 ...
 class KeyButton: UIButton {
     ...
     func enablePopup() {
         self.addTarget(self, action: #selector(showPopup), for: [.touchDown, .touchDragEnter, .touchUpOutside, .allTouchEvents])
     ...
     @objc func showPopup() {
         ...
         let popupWidth: CGFloat = self.bounds.width * 1.8 // initial 1.3
         ...
         for button in keyButtons {
             if let key = button.accessibilityLabel {
                 if key == "Shift" {
                     ...
                     // if keyboard type is letter use shiftmage else use a string
