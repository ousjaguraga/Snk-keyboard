...
class KeyButton: UIButton {
    ...
    /// Displays the popup view above the key.
    @objc func showPopup() {
        guard popupView == nil else { return }
        guard let windowView = self.window else { return }
        
        // Get the frame of the key button relative to the window
        let keyFrameInWindow = self.convert(self.bounds, to: windowView)
        
        let popupWidth: CGFloat = self.bounds.width * 1.5
        let popupHeight: CGFloat = self.bounds.height * 2.1
        
        // Initial popupX calculation
        var popupX = keyFrameInWindow.midX - popupWidth / 2
        let popupY = keyFrameInWindow.origin.y - popupHeight + 10
        
        // Adjust popupX to stay within window bounds
        if popupX < 0 {
            popupX = 0
        } else if popupX + popupWidth > windowView.bounds.width {
            popupX = windowView.bounds.width - popupWidth
        }
        
        popupView = UIView(frame: CGRect(x: popupX, y: popupY, width: popupWidth, height: popupHeight))
        popupView?.backgroundColor = self.backgroundColor
        popupView?.layer.cornerRadius = 5 // 5
        popupView?.layer.borderWidth = 1  // 1
        ...
        var keyCenterXInPopup = keyFrameInWindow.midX - popupX
        
        // Ensure the triangle is within the popup bounds
        keyCenterXInPopup = max(10, min(keyCenterXInPopup, popupWidth - 10))
        
        // Create a triangle path for the popup
        let trianglePath = UIBezierPath()
        trianglePath.move(to: CGPoint(x: keyCenterXInPopup - 10, y: popupHeight - 10))
        trianglePath.addLine(to: CGPoint(x: keyCenterXInPopup, y: popupHeight))
        trianglePath.addLine(to: CGPoint(x: keyCenterXInPopup + 10, y: popupHeight - 10))
        trianglePath.close()
        ...
        popupLabel = UILabel(frame: CGRect(x: 0, y: 0, width: popupWidth, height: popupHeight - 20))
...
class KeyboardViewController: UIInputViewController {
    ...
    func addSuggestionBar() {
        ...
        // Style the suggestion bar
        suggestionBar.backgroundColor = UIColor(red: 0.784, green: 0.784, blue: 0.804, alpha: 1.0) // Light gray color
        ...
        divider.backgroundColor = UIColor(red: 0.678, green: 0.678, blue: 0.698, alpha: 1.0) // Slightly darker gray
        suggestionBar.addSubview(divider)
        
        NSLayoutConstraint.activate([
            divider.leftAnchor.constraint(equalTo: suggestionBar.leftAnchor),
            divider.rightAnchor.constraint(equalTo: suggestionBar.rightAnchor),
            divider.bottomAnchor.constraint(equalTo: suggestionBar.bottomAnchor),
    ...
    func updateSuggestions() {
        ...
        // Find suggestions
        let matches = wordList.filter { $0.hasPrefix(currentWord) && $0 != currentWord }
        ...
        for suggestion in topSuggestions {
            ...
            if let prevButton = previousButton {
                button.leftAnchor.constraint(equalTo: prevButton.rightAnchor, constant: 6).isActive = true
                button.widthAnchor.constraint(equalTo: prevButton.widthAnchor).isActive = true
            } else {
                button.leftAnchor.constraint(equalTo: suggestionBar.leftAnchor, constant: 6).isActive = true
            }
            button.topAnchor.constraint(equalTo: suggestionBar.topAnchor, constant: 6).isActive = true
            button.bottomAnchor.constraint(equalTo: suggestionBar.bottomAnchor, constant: -6).isActive = true
            
            previousButton = button
        }
        previousButton?.rightAnchor.constraint(lessThanOrEqualTo: suggestionBar.rightAnchor, constant: -6).isActive = true
    }
    ...
    func addShiftRowKeys(_ rowKeys: [String], to rowView: UIView, keySpacing: CGFloat) {
        ...
        if let firstMiddleKey = middleKeyButtons.first {
            firstMiddleKey.leftAnchor.constraint(equalTo: shiftKeyButton.rightAnchor, constant: keySpacing * 2).isActive = true
        }
        ...
        if let lastMiddleKey = middleKeyButtons.last {
            deleteKeyButton.leftAnchor.constraint(equalTo: lastMiddleKey.rightAnchor, constant: keySpacing * 2).isActive = true
        }
    ...
    /// Updates the key colors based on the keyboard appearance (light/dark mode).
    func updateKeyColors(isDarkMode: Bool) {
        for button in keyButtons {
            if isDarkMode {
                if ["Shift", "Delete", "⌫", "Globe", "Return", "123", "ABC", "Dictation"].contains(button.accessibilityLabel ?? "") {
                    button.backgroundColor = UIColor(red: 0.333, green: 0.333, blue: 0.356, alpha: 1.0) // Dark gray
                    button.setTitleColor(.white, for: .normal)
                } else {
                    button.backgroundColor = UIColor(red: 0.223, green: 0.223, blue: 0.247, alpha: 1.0) // Darker gray
                    button.setTitleColor(.white, for: .normal)
        ...
        suggestionBar.backgroundColor = isDarkMode ? UIColor(red: 0.333, green: 0.333, blue: 0.356, alpha: 1.0) : UIColor(red: 0.784, green: 0.784, blue: 0.804, alpha: 1.0)
        suggestionButtons.forEach { button in
            button.backgroundColor = isDarkMode ? UIColor(red: 0.223, green: 0.223, blue: 0.247, alpha: 1.0) : .white
